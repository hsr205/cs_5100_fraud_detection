...
class IFModel:
    ...
    def detect(self, num_observations=16000):
        ...
        if num_observations > len(self.fraud_data_frame[self.fraud_data_frame[
                                                            'isFraud'] == 1]):  # if number of observations is greater than the number of anomalies in the dataset
            self.fraud_data_frame = self.fraud_data_frame.sort_values(by='isFraud',
                                                                      ascending=False)  # include all anomlies

        self.fraud_data_frame = self.fraud_data_frame.head(
            num_observations)  # only use limited amount to make analysis more efficient
        self.preprocess()
        ...
        iso_forest = IsolationForest(n_estimators=132, contamination=0.35, max_samples=0.15,
        ...
        self.fraud_data_frame['predictedFraud'] = self.fraud_data_frame['predictedFraud'].apply(
    ...
    def vis_2d(self):
        ...
        sns.scatterplot(data=self.fraud_data_frame, x='amount_norm', y='new_balance_origin_normalized',
                        hue='predictedFraud', palette={0: 'green', 1: 'red'}, s=100, alpha=0.6)
